...
def analyzeJob(job: Job) -> None:
    """
    Define Constants
    """
    CSV_DIR = os.path.join(job.result_dir, "csv")
    R_DIR = os.path.join(job.result_dir, "r-data-objects")
    CLEAR_OUTPUT = True
    LOG_LEVEL = "info"
    CLEAR_LOG = True
    ...
    """
    # Excel file
    excel_file = pd.ExcelFile(job.source)

    # Methods to be used
    use_methods = json.loads(job.methods)
